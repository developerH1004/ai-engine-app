...
export default function AdminPage() {
  ...
  async function searchByDate() {
    ...
    setLoading(true); setDateResults([]); setMessage('')
    const to = dateTo || dateFrom
    const toNext = new Date(to)
    toNext.setDate(toNext.getDate() + 1)
    const toNextStr = toNext.toISOString().slice(0, 10)

    const SUPABASE_URL = process.env.NEXT_PUBLIC_SUPABASE_URL
    const SUPABASE_KEY = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY

    // 신규
    const r1 = await fetch(`${SUPABASE_URL}/rest/v1/ai_products?select=id,product_name,manufacturer,category_main,created_at,updated_at,verification_status,parent_platform&created_at=gte.${dateFrom}T00:00:00&created_at=lt.${toNextStr}T00:00:00&order=created_at.desc`, {
      headers: { apikey: SUPABASE_KEY!, Authorization: `Bearer ${SUPABASE_KEY}` }
    })
    const newItems = await r1.json()

    // 업데이트
    const r2 = await fetch(`${SUPABASE_URL}/rest/v1/ai_products?select=id,product_name,manufacturer,category_main,created_at,updated_at,verification_status,parent_platform&updated_at=gte.${dateFrom}T00:00:00&updated_at=lt.${toNextStr}T00:00:00&created_at=lt.${dateFrom}T00:00:00&order=updated_at.desc`, {
      headers: { apikey: SUPABASE_KEY!, Authorization: `Bearer ${SUPABASE_KEY}` }
    })
    const updatedItems = await r2.json()

    const combined = [
      ...(Array.isArray(newItems) ? newItems.map((r: any) => ({ ...r, _type: '신규' })) : []),
      ...(Array.isArray(updatedItems) ? updatedItems.map((r: any) => ({ ...r, _type: '수정' })) : [])
    ]
    setDateResults(combined)
    setMessage(`${dateFrom} ~ ${to} : 신규 ${Array.isArray(newItems) ? newItems.length : 0}개 / 수정 ${Array.isArray(updatedItems) ? updatedItems.length : 0}개`)
    setLoading(false)
  }
  ...
  // ── 메인 UI ─────────────────────────────────────────────
  const S: React.CSSProperties = { minHeight: '100vh', background: '#0d1117', color: '#e6edf3', fontFamily: 'monospace' }
  const NAV: React.CSSProperties = { background: '#161b22', borderBottom: '1px solid rgba(184,150,64,0.2)', padding: '0 24px', display: 'flex', alignItems: 'center', gap: '0', height: '52px' }
  const CONTENT: React.CSSProperties = { maxWidth: '1200px', margin: '0 auto', padding: '24px' }
  ...
  return (
    <div style={S}>
      ...
      <div style={NAV}>
        ...
        {SECTIONS.map(s => (
          <button key={s} onClick={() => { setSection(s); setResults([]); setMessage('') }}
      ...
      <div style={CONTENT}>
        ...
        {section === '날짜 검색' && (
          <div>
            ...
            <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-end', marginBottom: '24px', flexWrap: 'wrap' }}>
              <div>
                <label style={{ display: 'block', color: '#888', fontSize: '12px', marginBottom: '6px' }}>시작일 (YYMMDD 또는 YYYY-MM-DD)</label>
                <input value={dateFrom} onChange={e => setDateFrom(e.target.value)}
                  placeholder="260520 또는 2026-05-20"
              ...
              <div>
                ...
                <input value={dateTo} onChange={e => setDateTo(e.target.value)}
                  placeholder="260524 또는 2026-05-24"
                  style={{ padding: '10px 14px', borderRadius: '8px', background: '#0d1117', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', fontSize: '13px', outline: 'none', width: '200px' }} />
              </div>
                    ...
                    {dateResults.map((r, i) => (
                      <tr key={r.id ?? i} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                        <td style={{ padding: '8px 14px' }}>
                          <span style={{ padding: '2px 8px', borderRadius: '99px', fontSize: '11px', fontWeight: 600, background: r._type === '신규' ? 'rgba(0,255,136,0.1)' : 'rgba(74,138,223,0.1)', border: `1px solid ${r._type === '신규' ? 'rgba(0,255,136,0.3)' : 'rgba(74,138,223,0.3)'}`, color: r._type === '신규' ? '#00ff88' : '#4a8adf' }}>
                            {r._type === '신규' ? '🆕 신규' : '🔄 수정'}
                          </span>
                        </td>
                        ...
                        <td style={{ padding: '8px 14px', color: '#888', fontFamily: 'monospace', fontSize: '12px', whiteSpace: 'nowrap' }}>
                          {r._type === '신규' ? (r.created_at || '').slice(0, 10) : (r.updated_at || '').slice(0, 10)}
                        </td>
        ...
        {section === '로그' && (
          <div>
            <h2 style={{ color: '#b89640', marginBottom: '24px' }}>📋 업데이트 로그</h2>
            <p style={{ color: '#666', fontSize: '13px' }}>Supabase → update_logs 테이블에서 최근 50개 조회합니다.</p>
            <button onClick={async () => {
              setLoading(true)
              const SUPABASE_URL = process.env.NEXT_PUBLIC_SUPABASE_URL
              const SUPABASE_KEY = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY
              const r = await fetch(`${SUPABASE_URL}/rest/v1/update_logs?select=*&order=created_at.desc&limit=50`, {
                headers: { apikey: SUPABASE_KEY!, Authorization: `Bearer ${SUPABASE_KEY}` }
              })
              const data = await r.json()
              setLogs(Array.isArray(data) ? data : [])
              setLoading(false)
            }} style={{ marginBottom: '16px', padding: '8px 20px', borderRadius: '8px', background: '#b89640', border: 'none', color: '#000', fontSize: '13px', fontWeight: 700, cursor: 'pointer' }}>
            ...
            <tr key={r.id}
              ...
              <td style={{ padding: '8px 14px' }}>
                <input type="checkbox" readOnly checked={selected.has(r.id)}
                  style={{ accentColor: '#ff4444', width: '14px', height: '14px' }} />
              </td>
              <td style={{ padding: '8px 14px', color: '#e6edf3', fontWeight: 500 }}>{r.product_name}</td>
              <td style={{ padding: '8px 14px', color: '#aaa' }}>{r.manufacturer || '-'}</td>
              ...
              <td style={{ padding: '8px 14px', color: '#4a8adf', fontSize: '11px', maxWidth: '200px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
